% Visual Reconstruction: complete data analysis (***least squares, separate***)
% _
...
%         19/02/2019, 12:50 / 01/04/2019, 11:45
...
for i = 1:num_subj
    ...
    % estimate model
    ITEM_est_1st_lvl_LS_S(SPM, est_mod, [2 3]);
    fprintf('successful!\n');
...
for i = 1:num_subj
    ...
    % perform decoding
    ITEM_dec_recon_LS_S(SPM, ROI_nii, c, con, reg);
    fprintf('done.\n');
...
for i = 1:num_subj
    ...
    con = 'sects-all';
    reg = 'V1-all';
    ITEM_mat = strcat(bids_dir,'/','sub-',subj_ids{i},'/anal/','glms-',MS_name,'/','glm-',GLM_names{1},'/','ITEM_dec_recon_LS_S','/','ITEM_',con,'_',reg,'.mat');
    load(ITEM_mat);
...
% open figure
figure('Name','Visual Reconstruction (LS-S)','Color',[1 1 1],'Position',[50 50 1600 900])
