function ITEM_dec_recon_LS_S_SVR(SPM, ROI, c, con, reg)
% _
% Decoding from Trials for Reconstruction (***least squares, separate [SVR]***)
...
%  Last edit: 02/04/2019, 15:00 (Vn/a)
...
%-------------------------------------------------------------------------%
load(strcat(SPM.swd,'/','ITEM_est_1st_lvl_LS_S','/','GLM1.mat'));

% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_LS_S_SVR: load');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_LS_S_SVR: estimate (1)');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_LS_S_SVR: estimate (2)');
...
for g = 1:s
    ...
    % Learn and apply reconstruction weights
    %---------------------------------------------------------------------%
    spm_progress_bar('Init',100,sprintf('Reconstructing predictors in Session %d...',g),'');
    i_pred = find(c);
    Y_pred = zeros(size(Y_out));
    for i = i_pred
        svm = svmtrain(Y_in(:,i), X_in, '-s 3 -t 0 -q');
        [Y_pred(:,i), acc_SVR, dv_SVR] = svmpredict(Y_out(:,i), X_out, svm, '-q');
        spm_progress_bar('Set',(find(i==i_pred)/sum(c))*100);
    end;
    clear acc_SVR dv_SVR
    
    % Extract true and reconstructed design
    %---------------------------------------------------------------------%
    Y_true  = Y_out(:,c==1);
    Y_recon = Y_pred(:,c==1);
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_LS_S_SVR: save');

% Create target directory
%-------------------------------------------------------------------------%
ITEM.swd = strcat(SPM.swd,'/','ITEM_dec_recon_LS_S_SVR','/');
