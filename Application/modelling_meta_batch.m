...
if strcmp(proc_mode,'prepare')
    ...
    for i = 1:num_subj
        ...
        % analysis folder
        anal_dir = strcat(bids_dir,'/','sub-',subj_ids{i},'/anal/');
        if ~exist(anal_dir,'dir'), mkdir(anal_dir); end;
        % model space
        MS_dir = strcat(bids_dir,'/','sub-',subj_ids{i},'/anal/','glms-',MS_name,'/');
        if ~exist(MS_dir,'dir'), mkdir(MS_dir); end;
        % single models
        for j = 1:num_mods
            GLM_dir = strcat(bids_dir,'/','sub-',subj_ids{i},'/anal/','glms-',MS_name,'/','glm-',GLM_names{j},'/');
...
if strcmp(proc_mode,'perform')
    ...
    for i = 1:num_subj
        for j = 1:num_mods
            ...
            filename = strcat(bids_dir,'/','sub-',subj_ids{i},'/anal/','glms-',MS_name,'/','sub-',subj_ids{i},'_glm-',GLM_names{j},'_design.mat');
            try
