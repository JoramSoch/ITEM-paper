...
for i = 1:numel(GLM_names)
    ...
    GLM_dir = strcat(work_dir,'/','sub-',subj_id,'/anal/','glms-',MS_name,'/','glm-',GLM_names{i},'/');
    ...
    for j = 1:num_sess
        ...
        if strcmp(MS_name,'test')
            if strcmp(GLM_names{i},'base-JS')
                mat_mat = strcat(work_dir,'/','sub-',subj_id,'/anal/','glms-',MS_name,'/','sub-',subj_id,'_glm-',GLM_names{i},'_run-0',num2str(j),'_matrix.mat');
                matlabbatch{1}.spm.stats.fmri_spec.sess(j).multi = {''};
                matlabbatch{1}.spm.stats.fmri_spec.sess(j).multi_reg{1} = mat_mat;
            else
                ons_mat = strcat(work_dir,'/','sub-',subj_id,'/anal/','glms-',MS_name,'/','sub-',subj_id,'_glm-',GLM_names{i},'_run-0',num2str(j),'_onsets.mat');
    ...
    % save batch file
    filename = strcat(work_dir,'/','sub-',subj_id,'/anal/','glms-',MS_name,'/','sub-',subj_id,'_glm-',GLM_names{i},'_design.mat');
